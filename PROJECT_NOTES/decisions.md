...
live at its tip, and `refs/pull/3/head` (66b119) still has the pre-scrub commit in its ancestry,
because PR head refs are server-managed and force-push/filter-repo cannot touch them. Confirmed via
`git ls-remote origin` after the rewrite. Closing that gap needs either a GitHub Support request to
purge those refs, or — the action that actually neutralizes it regardless of where a copy sits —
regenerating or deleting the key on InsForge's own dashboard. See open_items.
