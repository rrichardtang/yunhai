...
**[→ Try it live at yunhai.io](https://yunhai.io)**

YunHai is a full-stack AI travel planner: tell it your cities, dates, and taste, and it generates a day-by-day itinerary — real venues, real opening hours, real commute times — then keeps it internally consistent as you edit it.
...
- **Plans a full trip in one flow** — Cities → Activities → Arrange → Review → Trip Health, from a blank slate to a scheduled, bookable itinerary.
- **Auto-arranges the schedule** — drop activities across multiple cities and it schedules them into days with commute times and opening hours already respected.
- **Chats about the trip** — a concierge that can refine or swap an activity, or answer questions with live web search, grounded in what's actually on the itinerary.
- **Learns preferences over time** — state a preference once in chat and it surfaces in later planning and refinement, not just that session.
- **Tracks bookings automatically** — forward a flight/hotel/car confirmation email to a per-user address and it's parsed onto the trip; export the finished plan to Google Calendar.
- **Flags what's not ready** — Trip Health catches overlapping activities, missing times, and unbooked reservations before the trip does.
